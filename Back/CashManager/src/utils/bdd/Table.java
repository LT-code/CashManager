...
public class Table {
		...
		for(int i = 0; i < fields.length; i++) {
			res += 	"  " + (i < 1 ? "" : ", ") +
					...
					(fields[i].isUnique() ? "UNIQUE " : 
						(fields[i].isForeignKey() ? "not null" : ""));
					
			if(fields[i].isForeignKey())
				res += ", FOREIGN KEY (" + fields[i].getName() + ") REFERENCES " + fields[i].getAssociatedTable() + (fields[i].getKeyType() == TableFields.KEY_FOREIGN_ON_DELETE_CASCADE ? " ON DELETE CASCADE": "");
		}
    			...
    			for(int i = 0; i < tableCurrentTable.size(); i++) {
    				try {
    					...
    					if(e instanceof NullPointerException) {
    						if(!tableCurrentTable.get(i).get("Field").equals(fields[i].getName()))
    		    				return true;
    	    				if(!tableCurrentTable.get(i).get("Type").equals(fields[i].getTypeString()))
    	    					return true;
    	    				if(fields[i].isPrimaryKey())
    	    					if(!tableCurrentTable.get(i).get("Key").equals("PRI"))
    	    							return true;
    	    				if(fields[i].isUnique())
    	    					if(!tableCurrentTable.get(i).get("Key").equals("UNI"))
    	    							return true;	
    	    				if(fields[i].isForeignKey())
    	    					if(!tableCurrentTable.get(i).get("Key").equals("MUL"))
    	    							return true;
    	    				if(fields[i].isAutoGenerated())
    	    					if(!tableCurrentTable.get(i).get("Extra").equals("auto_increment"))
    	    							return true;
	...
	public boolean createTable(DBConnector db) throws SQLException, FailedDBConnection, ClassNotFoundException {    
    	boolean toReturn;
